...
- **The scoring code can be imported.** `scoreHarvest(harvest)` in `diagnose/score.mjs` returns the score, the nine tile results and the metrics as plain data. The report uses the same function. A CI check or a dashboard gets the same numbers as the page, and every `summary.json` records its schema version and the benchmark used.
