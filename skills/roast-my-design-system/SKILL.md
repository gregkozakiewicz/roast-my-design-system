...
4. **Generate the diagnosis page with the notes embedded, then deliver the same roast in chat** (same analysis, two places):
   ...
   - Compare against the benchmark the way the page does, and take the figures from `<skill-dir>/scripts/benchmark/benchmark.json`, never from memory: `stats.<metric>.median` is the median of the 34-repo core fleet; for a `shadcn` or `registry` repo use `slices.shadcn.stats.<metric>.median` instead, the 16 shadcn repos the page compares them with. If this repo is worse than its median, say so plainly.
