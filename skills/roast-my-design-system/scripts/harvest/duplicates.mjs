...
export function findDuplicates(components, uiDir = null, root = null, uiDirs = null) {
  ...
  if (root) {
    for (const d of exactDuplicates) {
      ...
      // A file called Button(.tsx exists somewhere; as a bare pattern it was an
      // unterminated group that took the whole scan down. Text, not regex.
      const escapeRe = (t) => t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const base = (f) => escapeRe(f.replace(/\.[jt]sx?$/, '').split('/').pop());
      ...
      const importsAliased = (fromFile, name, otherBase) => {
        ...
        return new RegExp(`import\\s*(?:type\\s*)?{[^}]*\\b${n}\\s+as\\s+\\w+[^}]*}\\s*from`).test(src)
          || new RegExp(`import\\s*(?:type\\s*)?{[^}]*\\b${n}\\b[^}]*}\\s*from\\s*['"][^'"]*${otherBase}['"]`).test(src);
      };
      const [a, b] = d.files;
