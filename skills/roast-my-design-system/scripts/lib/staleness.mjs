...
// A rules file a tool wrote for itself (a lint preset's rule pack, the
// Next.js agent block): its stale lines are the tool's, not the team's, so
// the report names the tool instead of saying "your agent rules".
const GENERATORS = [
  [/ultracite/i, 'Ultracite'],
  [/nextjs-agent-rules|NEXT-AGENTS-MD/, 'Next.js'],
  [/\bbiome\b/i, 'Biome'],
  [/generated by ([\w@/.-]+)/i, null],
];
function generatorOf(text) {
  const head = text.slice(0, 600);
  for (const [re, name] of GENERATORS) { const m = head.match(re); if (m) return name ?? m[1]; }
  return null;
}
...
function ruleTexts(root) {
  ...
  const push = (file, text) => {
    ...
    texts.push({ file, text, handWritten, ours: OURS_RE.test(handWritten), tool: generatorOf(text) });
  };
...
export function ruleStaleness(root, components, neverImportedNames = new Set(), fileSuffixes = []) {
  ...
  const findings = [];
  const seen = new Set();
  const add = (file, ref, kind, problem, tool = null) => {
    const key = `${ref}|${problem}`;
    if (seen.has(key) || findings.length >= MAX_FINDINGS) return;
    seen.add(key);
    findings.push({ file, ref, kind, problem, ...(tool ? { tool } : {}) });
  };
  ...
  for (const { file, text, handWritten, ours, tool } of ruleTexts(root)) {
    for (const m of text.matchAll(PATH_RE)) {
      ...
      add(file, ref, 'path', 'missing', tool);
    }
