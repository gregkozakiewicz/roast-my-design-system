...
export const ANTD = {
  ...
  advice: {
    ...
    promptSpacing: '- A helper that is not a component cannot call theme.useToken(): pass the token in as a parameter rather than moving the helper or calling the hook outside a component.\n- The token values above are the defaults; check the ConfigProvider theme for overrides (token.sizeUnit, token.margin) before converting. Convert only an exact match.\n- Space and Flex take size and gap as "small", "middle", "large"; prefer those for gaps between components.',
