...
- This is a shadcn install (style `new-york`, base colour zinc). The theme is a set of named rows in `app/globals.css`: background, foreground, primary, muted, border and the rest, each for light and dark. Change a colour there, never in a component.
